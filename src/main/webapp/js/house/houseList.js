	...
	function importExcel(){
	    ...
	    if(files.length<=0){
	    	...
	    	$.ajax({
		        ...
		        success : function(result) {
		        	var json = eval(result); //数组    
		        	if(json.house.length>0){
		        		art.dialog({icon:'succeed', title:'友情提示', drag:false, resize:false, content:'导入成功', ok:true,});
		        	}
		        }
		    });
	    }
			...
			if( confirm('您确定要导出吗？') ){
				...
				$.ajax({
			        ...
			        success : function(result) {
			        	var json = eval(result); //数组    
			        	if(json.house.length>0){
			        		art.dialog({icon:'succeed', title:'友情提示', drag:false, resize:false, content:'导出成功', ok:true,});
			        	}
			        }
			    });
			}
